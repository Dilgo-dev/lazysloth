...
export function SavedRequestsList({
  ...
  const { currentWorkspace, updateRequest, saveRequest } = useWorkspace();
  const [searchTerm, setSearchTerm] = useState("");
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editName, setEditName] = useState("");

  const filteredRequests =
    currentWorkspace?.requests.filter(
      (request) =>
  ...
  const handleCreateNewRequest = async () => {
    try {
      ...
      const newRequest = currentWorkspace?.requests.find(
        (r) => r.id === requestId
      );
      if (newRequest) {
                ...
                style={{
                  ...
                  borderColor:
                    selectedRequest?.id === request.id
                      ...
                      : "var(--border)",
                  borderWidth:
                    selectedRequest?.id === request.id ? "2px" : "1px",
                }}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center space-x-2 mb-2">
                      <span
                        ...
                        style={
                          methodColors[
                            request.method as keyof typeof methodColors
                          ]
                        }
                      >
                        {request.method}
